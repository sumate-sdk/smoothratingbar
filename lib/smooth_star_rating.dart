...
class _SmoothStarRatingState extends State<StarRating> {
  ...
  bool get isLinear => (widget.gradientLinear != null);
  bool get isRadial => (widget.gradientRadial != null);
  bool get isGradient => (isLinear || isRadial);

  Widget maskShade(Widget child) {
    if (isLinear) {
      return ShaderMask(
        shaderCallback: (rect) => widget.gradientLinear.createShader(rect),
        child: child,
      );
    } else if (isRadial) {
      return ShaderMask(
        shaderCallback: (rect) => widget.gradientRadial.createShader(rect),
        child: child,
      );
    } else {
      return child;
